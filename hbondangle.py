...
import itertools
import sys
InFileName = "C:/Users/Erik/Documents/comp research/pdb/good resolution/2PIT.pdb"
...
for carb in range(0,ty//6):
        ...
        #print("(" + str(dict_cen_benz[benzall]['cenx']) + ", " + str(dict_cen_benz[benzall]['ceny'])+ ", " + str(dict_cen_benz[benzall]['cenz']) + " )" )
        #print(dict_cen_benz[benzall]['norm_v'],dict_cen_benz[benzall]['resname'],dict_cen_benz[benzall]['resseq'],dict_cen_benz[benzall]['sn_range'])
        benzall+=1
ph+=1
#print("")
#print("Phenylalanine Rings")
#print("")
            ...
            if serial_number==dict_conect[con]['serial_number2']:
                for hal in range(len(dict_hethalogen)):
                    if dict_hethalogen[hal]['serial_number']==dict_conect[con]['serial_number1']:
                        ...
                        #print(dict_hethal[n]['fullname'])
                        #print(dict_hetcarbon[n]['fullname'])
                        #print(n)
                        #print('')
                        n = n + 1
                        break
        ...
        for line in InFile:
                ...
                if record_type == 'ATOM  ':
                        ...
                        if distance < 4.0: # if length is less than 4.0 
                                ...
                                print (InFileName, dict_hethalogen[j]['serial_number'], dict_hethalogen[j]['fullname'], dict_hethalogen[j]['resname'], dict_hethalogen[j]['atom_type'], dict_hetcarbon[j]['serial_number'], dict_hetcarbon[j]['fullname'], dict_hetcarbon[j]['resname'],dict_hetcarbon[j]['atom_type'], dict_atom[s]['serial_number'],dict_atom[s]['fullname'], dict_atom[s]['resname'],dict_atom[s]['resseq'], dict_atom[s]['atom_type'], "Angle : %5.1f degrees distance:%5.1f" %(Angle, distance))
                                #OutFile.write(InFileName, dict_hethalogen[j]['serial_number'], dict_hethalogen[j]['fullname'], dict_hethalogen[j]['resname'], dict_hethalogen[j]['chainid'], dict_hethalogen[j]['resseq'],dict_hetcarbon[j]['serial_number'], dict_hetcarbon[j]['fullname'], dict_hetcarbon[j]['resname'], dict_hetcarbon[j]['chainid'],dict_hetcarbon[j]['reseq'], dict_atom[s]['serial_number'],dict_atom[s]['fullname'], dict_atom[s]['resname'],dict_atom[s]['chainid'],dict_atom[s]['reseq'] "Angle : %5.1f degrees distance:%5.1f" %(Angle, distance) + '\n')
...
for ent in range(0,w):
    for b in range(0,benzall):
        if rings and benz_dis(dict_cen_benz[b]['cenx'],dict_hethal[ent]['x'],
                              dict_cen_benz[b]['ceny'],dict_hethal[ent]['y'],
                              dict_cen_benz[b]['cenz'],dict_hethal[ent]['z']) <= 6:
                        print(InFileName, 
                              dict_hethal[ent]['serial_number'],
                              dict_hethal[ent]['fullname'], 
                              dict_hethal[ent]['resname'], 
                              dict_hethal[ent]['atom_type'],
                              dict_cen_benz[b]['resname'],
                              dict_cen_benz[b]['resseq'], 
                              dict_cen_benz[b]['sn_range'],
                              'Benz-Hal-C Angle: ',
                              bh_bond_angle([dict_hetcarbon[ent]['x']-dict_hethal[ent]['x'],
                              ...
                              'Bond-Bplane Angle: ',
                              plane_angle(dict_cen_benz[b]['cenx'],dict_hethal[ent]['x'],
                              dict_cen_benz[b]['ceny'],dict_hethal[ent]['y'],
                              dict_cen_benz[b]['cenz'],dict_hethal[ent]['z'],
                              dict_cen_benz[b]['norm_v']),
                              "Bond-Bplane Dis: ",
                              round(benz_dis(dict_cen_benz[b]['cenx'],dict_hethal[ent]['x'],
                              dict_cen_benz[b]['ceny'],dict_hethal[ent]['y'],
                              dict_cen_benz[b]['cenz'],dict_hethal[ent]['z']),2))
